...
inherit autotools base bash-completion-r1 gnome.org linux-info systemd user readme.gentoo toolchain-funcs vala virtualx udev eutils ubuntu-versionator

MY_PN="network-manager"
MY_P="${MY_PN}_${PV}"
...
src_prepare() {
	# 'adhoc_use_wpa_rsn_part2.patch' fails to compile because of undeclared 'eiter', guess it's a$
	sed -i '/adhoc_use_wpa_rsn_part1.patch/d' "${WORKDIR}/debian/patches/series" || die
	sed -i '/adhoc_use_wpa_rsn_part2.patch/d' "${WORKDIR}/debian/patches/series" || die
