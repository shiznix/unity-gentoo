...
case ${PV} in
	*)
		...
		MY_P=${QT5_MODULE}-opensource-src
		# Fetch Ubuntu monolithic qtbase patchset tarball #
		if [[ ${QT5_MODULE} == qtbase ]]; then
			URELEASE="yakkety"
			UVER_PREFIX="+dfsg"
			UVER="3ubuntu6"
			UVER_SUFFIX="~4"
			SRC_URI="mirror://unity/pool/main/q/qtbase-opensource-src/${MY_P}_${PV}${UVER_PREFIX}.orig.tar.xz
				mirror://unity/pool/main/q/qtbase-opensource-src/${MY_P}_${PV}${UVER_PREFIX}-${UVER}${UVER_SUFFIX}.debian.tar.xz"
		else
			SRC_URI="https://download.qt.io/official_releases/qt/${PV%.*}/${PV}-1/submodules/${MY_P}-${PV}-1.tar.xz"
		fi
